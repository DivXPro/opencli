...
ToyCLI has five extension paths. Pick the path based on where you want the source code to live and how you want commands to be shared.

| Goal | Use | Source location | Command surface |
|------|-----|-----------------|-----------------|
| Build a personal website command in your own Git repo | Local plugin | Your project directory, symlinked into `~/.toycli/plugins/` | `toycli <plugin> <command>` |
| Quickly draft a private adapter on this machine | User adapter | `~/.toycli/clis/<site>/<command>.js` | `toycli <site> <command>` |
| Edit an official adapter locally | Adapter override | `~/.toycli/clis/<site>/` | `toycli <site> <command>` |
| Publish or install third-party commands | Plugin | Git repo, installed into `~/.toycli/plugins/` | `toycli <plugin> <command>` |
| Wrap an existing local binary | External CLI | `~/.toycli/external-clis.yaml` | `toycli <tool> ...` |
...
`plugin install file://...` creates a symlink under `~/.toycli/plugins/`. Your source files stay in your project directory, so edits and commits happen there.

This is the recommended path for custom commands you own long-term.

## Private adapters in `~/.toycli/clis`
...
`--seed-args` is only used when the fixture has no `args`. Once the fixture is written, `toycli browser verify` reads args from `~/.toycli/sites/<site>/verify/<command>.json`.
...
Files in `~/.toycli/clis/<site>/<command>.js` override packaged adapters with the same `site/command` on this machine. `toycli browser verify <site>/<command>` also runs the local override, so a passing local verify does not prove that the packaged adapter was changed.
...
After copying a local fix into the repository for a PR, remove the local copy or run `toycli adapter reset <site>` after merge. Otherwise the local file keeps shadowing future package updates. `toycli doctor` warns when it detects this shadowing.
...
For a Git-hosted plugin collection, declare sub-plugins in `toycli-plugin.json` and install from GitHub:
...
Use external CLI registration when the command already exists as a binary on your machine and you want it available through `toycli`.
