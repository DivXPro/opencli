...
OpenCLI 支持对任意站点做持续实时监听：扩展在 Chrome 中拦截网络响应、DOM 变化或 CDP 事件，将事件以 `listener-event` 形式推送到 daemon 的 EventBus，外部应用通过 HTTP/SSE 订阅，数据按 `listenerId` 隔离，多个订阅者互不混淆。
...
同一 `site/adapter:listenerId` 同时只允许一个活跃监听。重复 `start` 不会创建第二个标签页，会返回 `already-running`。需要重启时用 `opencli listener restart`。
