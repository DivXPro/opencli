...
ToyCLI 将**任何网站**或 **Electron 应用**变成命令行界面 — Bilibili、知乎、小红书、Twitter/X、Reddit、YouTube、Antigravity 等 — 基于浏览器会话复用和 AI 原生发现。
...
```bash
toycli list                              # 查看所有命令
toycli hackernews top --limit 5          # 公开 API，无需浏览器
toycli bilibili hot --limit 5            # 浏览器命令
toycli zhihu hot -f json                 # JSON 输出
```
...
```bash
toycli bilibili hot -f table   # 默认：终端表格
toycli bilibili hot -f json    # JSON
toycli bilibili hot -f yaml    # YAML
...
toycli [Tab]          # 补全站点名称（bilibili、zhihu、twitter...）
toycli bilibili [Tab] # 补全命令（hot、search、me、download...）
```
...
- [扩展 ToyCLI：自定义命令、plugin 和 external CLI](/zh/guide/extending-opencli)
- [所有适配器](/zh/adapters/)
