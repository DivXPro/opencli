...
| Command | Description |
|---------|-------------|
| `toycli stackoverflow hot` | Hot questions |
| `toycli stackoverflow search <query>` | Search questions |
| `toycli stackoverflow bounties` | Questions with active bounties |
| `toycli stackoverflow unanswered` | Unanswered questions |
| `toycli stackoverflow read <id>` | Read a question with answers and comments |
| `toycli stackoverflow user <name>` | Find users by display name (highest reputation first) |
| `toycli stackoverflow tag <tag>` | List questions tagged with a given tag (most active first) |
| `toycli stackoverflow related <id>` | List questions related to a given question id |
...
```bash
# Hot questions
toycli stackoverflow hot --limit 10

# Search questions
toycli stackoverflow search "async await" --limit 20

# Active bounties
toycli stackoverflow bounties --limit 10

# Unanswered questions
toycli stackoverflow unanswered --limit 10

# Tagged questions (id feeds stackoverflow read)
toycli stackoverflow tag rust --limit 10

# Related questions for a given question id (rows feed stackoverflow read)
toycli stackoverflow related 11227809 --limit 10
toycli stackoverflow related 11227809 --sort votes --limit 5

# User profile search (returns userId/profile URL rows)
toycli stackoverflow user "Jon Skeet" --limit 5

# Read a question with answers and comments
toycli stackoverflow read 11227809
toycli stackoverflow read 11227809 --answers-limit 3 --comments-limit 5

# JSON output
toycli stackoverflow hot -f json
toycli stackoverflow read 11227809 -f json
```
...
- `tag` returns question rows with `id`, so agents can call
  `toycli stackoverflow read <id>` without parsing URLs. `user` returns
