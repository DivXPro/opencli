...
| Command | Description |
|---------|-------------|
| `toycli paperreview submit` | Submit a PDF to paperreview.ai for review |
| `toycli paperreview review` | Fetch a review by token |
| `toycli paperreview feedback` | Send feedback on a completed review |

## Usage Examples

```bash
# Validate a local PDF without uploading it
toycli paperreview submit ./paper.pdf --email you@example.com --venue RAL --dry-run true

# Request an upload slot but stop before the actual upload
toycli paperreview submit ./paper.pdf --email you@example.com --venue RAL --prepare-only true

# Submit a paper for review
toycli paperreview submit ./paper.pdf --email you@example.com --venue RAL -f json

# Check the review status or fetch the final review
toycli paperreview review tok_123 -f json

# Submit feedback on the review quality
toycli paperreview feedback tok_123 --helpfulness 4 --critical-error no --actionable-suggestions yes
```
