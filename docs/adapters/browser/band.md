...
| Command | Description |
|---------|-------------|
| `toycli band bands` | List all Bands you belong to |
| `toycli band posts <band_no>` | List posts from a Band |
| `toycli band post <band_no> <post_no>` | Export full post content including nested comments |
| `toycli band mentions` | Show notifications where you were @mentioned |
...
toycli band posts 12345678 --limit 10

# Export a post with comments
toycli band post 12345678 987654321

# Export post body only (skip comments)
toycli band post 12345678 987654321 --comments false

# Export post and download attached photos
toycli band post 12345678 987654321 --output ./band-photos

# Show recent @mention notifications
toycli band mentions --limit 20

# Show only unread mentions
toycli band mentions --unread true

# Show all notification types
toycli band mentions --filter all
```
