...
| Command | Description |
|---------|-------------|
| `toycli google search <keyword>` | Search Google and extract results from the page |
| `toycli google suggest <keyword>` | Get Google search suggestions |
| `toycli google news [keyword]` | Get Google News headlines (top stories or search) |
| `toycli google trends` | Get Google Trends daily trending searches |
...
```bash
# Search Google
toycli google search "typescript tutorial" --limit 10

# Get search suggestions
toycli google suggest python

# Get top news headlines
toycli google news --limit 5

# Search news for a topic
toycli google news "artificial intelligence" --limit 10 --lang en --region US

# Get trending searches in Japan
toycli google trends --region JP --limit 10

# Output as JSON
toycli google search "machine learning" -f json
```
