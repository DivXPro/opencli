...
| Command | Description |
|---------|-------------|
| `toycli hltv search <query>` | Search players, teams, events, and articles |
| `toycli hltv player-summary` | Read a player's summary page and complete stats link |
| `toycli hltv player-matches` | Read a player's stats Matches tab |
| `toycli hltv player-form` | Aggregate recent player maps by summary, map, and opponent |
| `toycli hltv player-map-pool` | Aggregate a player's recent maps into map-pool buckets |
| `toycli hltv player-vs-team <team>` | Filter a player's maps against one team |
| `toycli hltv player-teammate-impact <playerA> <playerB>` | Compare shared and non-shared map samples for two teammates |
| `toycli hltv player-duel <playerA> <playerB>` | Compare two players on shared maps, including direct kills when available |
| `toycli hltv match-map <match>` | Read all player rows from one mapstats page |
| `toycli hltv match-series <match>` | Expand a BO1/BO3/BO5 into summary, map, and player rows |
| `toycli hltv team-matches <team>` | Read recent team map results |
| `toycli hltv team-map-pool <team>` | Read a team's visible map-pool stats |
| `toycli hltv event-matches <event>` | Read stats match rows for one event |

## Usage Examples

```bash
# Search and discover HLTV entity links
toycli hltv search niko --limit 5

# Read player pages and recent form
toycli hltv player-summary --player 3741/niko
toycli hltv player-matches --player 3741/niko --limit 10 -f json
toycli hltv player-form --player 19230/m0nesy --limit 30

# Compare two players
toycli hltv player-duel 3741/niko 21167/donk
toycli hltv player-duel 19230/m0nesy 3741/niko --mode history --limit 10

# Expand a series or inspect one mapstats URL
toycli hltv match-series https://www.hltv.org/stats/matches/126993/spirit-vs-falcons
toycli hltv match-map "https://www.hltv.org/stats/matches/mapstatsid/231594/falcons-vs-natus-vincere" -f json

# Team and event views
toycli hltv team-matches 6667/falcons --limit 10
toycli hltv team-map-pool 11283/falcons
toycli hltv event-matches 8301 --limit 10
```
