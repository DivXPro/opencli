...
| `src/commands/listener.ts` | Create | `opencli listener start/stop/stream/list/status/history/restart` implementations |
...
// Tests must opt in by setting OPENCLI_DAEMON_PORT before import.
const PORT = Number(process.env.OPENCLI_DAEMON_PORT_TEST ?? '19831');
...
### Task 13: Implement `opencli listener` commands
...
**Files:**
- Modify: `/Users/huhui/.opencli/clis/buyin/live-products.js`
...
```bash
git add /Users/huhui/.opencli/clis/buyin/live-products.js
git commit -m "feat(buyin): declare comments network listener on live-products adapter"
```

(Note: this file lives outside the repo in `~/.opencli/clis/`; if it shouldn't be committed there, copy the snippet into the in-repo `clis/buyin/live-products.js` if one exists. Check: `ls clis/buyin/`.)
...
OpenCLI 支持对任意站点做持续实时监听：扩展在 Chrome 中拦截网络响应、DOM 变化或 CDP 事件，
...
会返回 `already-running`。需要重启时用 `opencli listener restart`。
```
