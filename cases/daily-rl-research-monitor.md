...
toycli arxiv recent cs.LG --limit 30 -f json > /tmp/lg.json
toycli arxiv recent cs.AI --limit 30 -f json > /tmp/ai.json
...
toycli openreview venue "NeurIPS 2025 oral" --limit 50 -f json > /tmp/neurips.json

# 3. Hugging Face Daily Papers (community-upvoted research)
toycli hf top --period daily --limit 20 -f json > /tmp/hf.json
```
...
- Four pages of clicking and scrolling collapses into four `toycli` calls.
...
- Zero token cost on the ToyCLI side. The only paid step is the digest call at the end.
