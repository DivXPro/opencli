...
ToyCLI 可以用同一套 CLI 做三类事情：

- **直接使用现成适配器**：B站、知乎、小红书、Twitter/X、Reddit、HackerNews 等 [100+ 站点](#内置命令) 开箱即用。
- **让 AI Agent 操作任意网站**：在你的 AI Agent（Claude Code、Cursor 等）中安装 `toycli-browser` skill，Agent 就能用你的已登录浏览器导航、点击、输入/填充、提取任意网页内容。
- **把新网站写成 CLI**：用 `toycli browser` 原语 + `toycli-adapter-author` skill，从站点侦察、API 发现、字段解码到 `toycli browser verify` 一条龙。

除了网站能力，ToyCLI 还是一个 **CLI 枢纽**：你可以把 `gh`、`docker`、`longbridge`、`tg`、`discord`、`wx`、`ntn`（Notion）等本地工具统一注册到 `toycli` 下，也可以通过桌面端适配器控制 Cursor、Trae CN、Codex、Antigravity、ChatGPT、Trae SOLO 等 Electron 应用。

## 快速开始

### 1. 安装 ToyCLI

如果你是在自己的电脑上使用，优先安装 **ToyCLIApp**。它会内置
ToyCLI runtime，帮你安装 / 修复受管理的 `toycli` 命令，并提供系统托盘
UI 来做环境诊断、更新、浏览器登录态保活和网页转 Markdown。

**方式 A — ToyCLIApp（macOS / Windows 推荐）：**
从 <https://toycli.info/download> 下载最新版 App，安装后打开一次，在
System 页面安装或修复 `toycli` 命令。

**方式 B — npm 全局安装（纯 CLI / CI / 服务器）：**
通过 npm 安装时，ToyCLI 要求 **Node.js >= 20**。
...
ToyCLI 通过轻量 Browser Bridge 扩展和本地微型 daemon 与 Chrome/Chromium 通信。daemon 会按需自动启动。

**方式 A — Chrome Web Store（推荐）：**
在 [Chrome Web Store](https://chromewebstore.google.com/detail/toycli/ildkmabpimmkaediidaifkhjpohdnifk) 安装 **ToyCLI** 扩展。

**方式 B — 手动安装：**
1. 到 GitHub [Releases 页面](https://github.com/toy-box/toycli/releases) 下载最新的 `toycli-extension-v{version}.zip`。
...
- `toycli list` 查看当前所有命令
- `toycli <site> <command>` 调用内置或生成好的适配器
- `toycli external register mycli` 把本地 CLI 接入同一发现入口
- `toycli doctor` 处理浏览器连通性问题

## 扩展 ToyCLI

如果你想新增自己的命令，先看 [扩展 ToyCLI](./docs/zh/guide/extending-opencli.md)。README 只保留入口；目录结构、源码管理方式和安装命令放在文档里。

| 需求 | 推荐路径 |
|------|----------|
| 把个人网站命令放在自己的 Git repo | `toycli plugin create` + `toycli plugin install file://...` |
| 快速写一个本机私人 adapter | `toycli browser init <site>/<command>`，放在 `~/.toycli/clis/` |
| 本地修改官方 adapter | `toycli adapter eject <site>` + `toycli adapter reset <site>` |
| 发布或安装第三方命令 | `toycli plugin install github:user/repo` |
| 包装已有本机 binary | `toycli external register <name>` |

## 给 AI Agent

ToyCLI 的 browser 命令是给 AI Agent 用的——不是手动执行的。把 skill 安装到你的 AI Agent（Claude Code、Cursor 等）中，Agent 就能用你的已登录 Chrome 会话替你操作网站。
...
|-------|---------|-------------------|
| **toycli-adapter-author** | 为新站点写可复用适配器，或给已有站点添加命令 | "帮我做一个抖音热门的适配器" / "帮我做一个抓取这个页面热帖的命令" |
| **toycli-autofix** | 内置命令失败时修复已有适配器 | "`toycli zhihu hot` 返回空了，修一下" |
| **toycli-browser** | 实时驱动 Chrome 页面——导航、填表单、点击、抓取 | "帮我看看小红书的通知" / "帮我填一下这个表单" / "用浏览器命令抓取这个页面" |
| **toycli-browser-sitemap** | 使用站点 sitemap 上下文来操作浏览器任务 | "用 sitemap 帮我少走弯路地操作这个网站" |
| **toycli-sitemap-author** | 创建或更新面向浏览器 Agent 的站点 sitemap | "把刚发现的稳定流程记录到这个站点的 sitemap" |
| **toycli-usage** | 所有命令和站点的快速参考 | "ToyCLI 有哪些 Twitter 相关的命令？" |

### 工作原理

安装 `toycli-browser` skill 后，你的 AI Agent 可以：
...
Agent 在内部自动处理所有 `toycli browser` 命令——你只需用自然语言描述想做的事。

**Skill 参考文档：**
- [`skills/toycli-browser/SKILL.md`](./skills/toycli-browser/SKILL.md) — 实时驱动 Chrome（导航、填表单、点击、抓取）
- [`skills/toycli-browser-sitemap/SKILL.md`](./skills/toycli-browser-sitemap/SKILL.md) — 操作浏览器任务时消费 sitemap 上下文
- [`skills/toycli-sitemap-author/SKILL.md`](./skills/toycli-sitemap-author/SKILL.md) — 创建或更新站点 sitemap 知识
- [`skills/toycli-adapter-author/SKILL.md`](./skills/toycli-adapter-author/SKILL.md) — 给新站点写适配器，全流程
- [`skills/toycli-autofix/SKILL.md`](./skills/toycli-autofix/SKILL.md) — 修复已有适配器
- [`skills/toycli-usage/SKILL.md`](./skills/toycli-usage/SKILL.md) — 命令和站点参考
...
`toycli browser` 命令必须紧跟一个 `<session>` 位置参数。`toycli browser work open <url>` 和 `toycli browser work tab new [url]` 都会返回 target ID。`toycli browser work tab list` 用来查看当前已存在 tab 的 target ID，再通过 `--tab <targetId>` 把命令明确路由到某个 tab。`tab new` 只会新建 tab，不会改变默认浏览器目标；只有显式执行 `tab select <targetId>`，才会把该 tab 设为同一 session 后续未指定 target 的默认目标。

## 为新站点写适配器

当你需要的网站还没覆盖时，用 `toycli-adapter-author` skill，全流程：
...
4. **字段解码** + 设计输出列
5. `toycli browser recon analyze <url>` → `toycli browser recon init <site>/<name>` → 写适配器 → `toycli browser recon verify <site>/<name>`
6. 站点知识沉到 `~/.toycli/sites/<site>/`，下次同站点直接吃缓存

## 配置

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `TOYCLI_DAEMON_PORT` | `19825` | daemon-extension 通信端口 |
| `TOYCLI_WINDOW` | 命令默认值 | 设为 `foreground` 或 `background` 来覆盖 Browser Bridge 窗口位置。浏览器型命令也支持 `--window <foreground\|background>` |
| `TOYCLI_BROWSER_CONNECT_TIMEOUT` | `45` | 浏览器连接超时（秒） |
| `TOYCLI_BROWSER_COMMAND_TIMEOUT` | `60` | 单个浏览器命令超时（秒） |
| `TOYCLI_CDP_ENDPOINT` | — | Chrome DevTools Protocol 端点，用于远程浏览器或 Electron 应用 |
| `TOYCLI_CDP_TARGET` | — | 按 URL 子串过滤 CDP target（如 `detail.1688.com`） |
| `TOYCLI_VERBOSE` | `false` | 启用详细日志（`-v` 也可以） |
| `DEBUG_SNAPSHOT` | — | 设为 `1` 输出 DOM 快照调试信息 |

`toycli browser *` 必须紧跟一个 `<session>` 位置参数，默认使用前台窗口，并保留该 session 的 tab lease，直到你手动执行 `toycli browser <session> close` 或等空闲超时。浏览器型 adapter 默认使用后台 adapter 窗口并在命令结束后释放一次性 tab lease；如果需要调试最终页面，可以传 `--window foreground --keep-tab true`。

## 内置命令

运行 `toycli list` 查看完整注册表。
...
注册自定义本地 CLI：`toycli external register <name>`；查看所有：`toycli external list`。
...
toycli bilibili download BV1xxx --quality 1080p  # 指定画质
...
`toycli xiaoyuzhou download` 和 `transcript` 需要本地小宇宙凭证：`~/.toycli/xiaoyuzhou.json`。
...
```bash
toycli list -f yaml            # 用 YAML 列出命令注册表
toycli bilibili hot -f table   # 默认：富文本表格
toycli bilibili hot -f json    # JSON（适合传给 jq 或者各类 AI Agent）
toycli bilibili hot -f yaml    # YAML（更适合人类直接阅读）
toycli bilibili hot -f md      # Markdown
toycli bilibili hot -f csv     # CSV
toycli bilibili hot -v         # 详细模式：展示管线执行步骤调试信息
```

## 退出码

toycli 遵循 Unix `sysexits.h`，CI / 脚本可按失败模式分支：`0` 成功、`66` 无数据、`69` Browser Bridge 未连接、`75` 超时、`77` 需要认证、`78` 配置错误、`130` Ctrl-C。完整参考：[docs/zh/guide/exit-codes.md](./docs/zh/guide/exit-codes.md)。

## 插件

通过社区贡献的插件扩展 ToyCLI。插件使用与内置命令相同的 JS 格式，启动时自动发现。

```bash
toycli plugin install github:user/toycli-plugin-my-tool  # 安装
toycli plugin list                                         # 查看已安装
toycli plugin update my-tool                               # 更新到最新
toycli plugin update --all                                 # 更新全部已安装插件
toycli plugin uninstall my-tool                            # 卸载
```

当 plugin 的版本被记录到 `~/.toycli/plugins.lock.json` 后，`toycli plugin list` 也会显示对应的短 commit hash。

| 插件 | 类型 | 描述 |
|------|------|------|
| [toycli-plugin-github-trending](https://github.com/ByteYue/toycli-plugin-github-trending) | JS | GitHub Trending 仓库 |
| [toycli-plugin-hot-digest](https://github.com/ByteYue/toycli-plugin-hot-digest) | JS | 多平台热榜聚合 |
| [toycli-plugin-juejin](https://github.com/Astro-Han/toycli-plugin-juejin) | JS | 稀土掘金热门文章 |
| [toycli-plugin-vk](https://github.com/flobo3/toycli-plugin-vk) | JS | VK (VKontakte) 动态、信息流和搜索 |
...
- **"Extension not connected" 报错**
  - 确保你已从 [Chrome Web Store](https://chromewebstore.google.com/detail/toycli/ildkmabpimmkaediidaifkhjpohdnifk) 安装 ToyCLI 扩展，且在 `chrome://extensions` 中**已启用**。
...
- **Node API 错误 / 缺少 `fetch` / 旧 Node 启动即崩**
  - ToyCLI 要求 **Node.js >= 20**。先执行 `node --version`，如果版本过低先升级，再重试命令。
- **Daemon 问题**
