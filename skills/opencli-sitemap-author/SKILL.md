---
name: toycli-sitemap-author
description: Use when creating or maintaining ToyCLI site sitemaps: agent-facing navigation, page-state, action, workflow, API-reference, pitfall, and fallback knowledge for a website. Use after browser exploration discovers durable site context, when a sitemap is stale, or when promoting local site knowledge into the repo.
allowed-tools: Bash(toycli:*), Read, Edit, Write, Grep
---

# toycli-sitemap-author

You are authoring a **task execution graph for agents**, not an SEO sitemap. The artifact should help an agent using `toycli browser` decide where it is, what path to take next, which ToyCLI adapter to prefer, and how to recover when the page disagrees with memory.
...
- **Local overlay**: `~/.toycli/sites/<site>/sitemap/`
...
2. **Verify reality**: use `toycli browser <session> state`, `find`, `network`, and `analyze`; browser state is truth. If you just completed an `toycli-adapter-author` session for this site, start from the retained browse trace under `~/.toycli/sites/<site>/traces/` as seed evidence instead of re-discovering the path from zero.
...
5. **Write local draft**: update `~/.toycli/sites/<site>/sitemap/...` unless explicitly promoting to repo.
...
evidence: toycli browser <cmd> or trace:<path>
```
...
- **Best path**: prefer existing `toycli <site> <command>` adapter if it covers the goal.
...
```yaml
on_adapter_fail:
  - adapter_health_update: toycli twitter post -> suspect
  - toycli browser state (verify current page)
...
- Drafts go inside `sitemap/draft-<topic>.md`, not `~/.toycli/sites/<site>/sitemap.draft.md` at the parent level — the latter is invisible to `toycli browser` sitemap availability detection.
