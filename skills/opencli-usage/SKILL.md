...
```bash
# npm global
npm install -g @toy-box/opencli          # binary: opencli, requires Node >= 21
...
Adapters import only `@toy-box/opencli/registry` and `@toy-box/opencli/errors`. `columns` must align 1:1 (in name and order) with keys of the object returned by `func`. For the full workflow see `opencli-adapter-author`.
