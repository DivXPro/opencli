...
describe('node network proxy decisions', () => {
  ...
  it('bypasses proxies for loopback addresses', () => {
    ...
    expect(decideProxy(new URL('http://127.0.0.1:29825/status'), env)).toEqual({ mode: 'direct' });
    expect(decideProxy(new URL('http://localhost:29825/status'), env)).toEqual({ mode: 'direct' });
    expect(decideProxy(new URL('http://[::1]:29825/status'), env)).toEqual({ mode: 'direct' });
  });
