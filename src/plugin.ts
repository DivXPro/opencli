...
export function validatePluginStructure(pluginDir: string): ValidationResult {
  ...
  if (hasTs) {
    ...
    if (!fs.existsSync(pkgJsonPath)) {
      errors.push('Plugin contains .ts files but no package.json. A package.json with "type": "module" and "@toy-box/opencli" peer dependency is required for TS plugins.');
    } else {
      try {
...
function linkHostOpencli(pluginDir: string): void {
  try {
    ...
    const targetLink = path.join(pluginDir, 'node_modules', '@toy-box', 'opencli');
