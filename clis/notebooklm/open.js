...
import { CliError, EmptyResultError } from '@toy-box/opencli/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new CliError('NOTEBOOKLM_OPEN_FAILED', `NotebookLM notebook "${notebookId}" did not open in the adapter session`, 'Run `toycli notebooklm list -f json` first and pass a valid notebook id.');
        }
        ...
        if (!current) {
            throw new EmptyResultError('toycli notebooklm open', 'NotebookLM notebook metadata was not found after navigation.');
        }
        return [current];
    },
});
